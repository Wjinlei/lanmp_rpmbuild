...
EOF
    sed -i "s|^prefix={php-fpm_location}$|prefix=${php53_location}|g" php53
}
...
_create_spec(){
    cat > ~/rpmbuild/SPECS/php53.spec << EOF
Name:           hws-php53
Version:        5.3.29
Release:        1%{?dist}
Summary:        PHP 5.3.29
...
%prep
tar zxf \$RPM_SOURCE_DIR/${php53_filename}.tar.gz

%build
cd ${php53_filename}
./configure --prefix=${php53_location} \
...
install -D -m 0755 \$RPM_SOURCE_DIR/php53 \$RPM_BUILD_ROOT/etc/init.d/php53
install -D -m 0644 \$RPM_SOURCE_DIR/default.conf \$RPM_BUILD_ROOT/${php53_location}/etc/default.conf
install -D -m 0644 \$RPM_BUILD_DIR/${php53_filename}/php.ini-production \$RPM_BUILD_ROOT/${php53_location}/etc/php.ini
...
rpmbuild_php53(){
    ...
    _create_fpm_config
    _create_spec
    rpmbuild -bb ~/rpmbuild/SPECS/php53.spec
}
