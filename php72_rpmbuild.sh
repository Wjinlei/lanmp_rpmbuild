...
EOF
    sed -i "s|^prefix={php-fpm_location}$|prefix=${php72_location}|g" php72
}
...
_create_spec(){
    cat > ~/rpmbuild/SPECS/php72.spec << EOF
Name:           hws-php72
Version:        7.2.27
Release:        1%{?dist}
Summary:        PHP 7.2.27
...
%prep
tar zxf \$RPM_SOURCE_DIR/${php72_filename}.tar.gz

%build
cd ${php72_filename}
./configure --prefix=${php72_location} \
...
install -D -m 0755 \$RPM_SOURCE_DIR/php72 \$RPM_BUILD_ROOT/etc/init.d/php72
install -D -m 0644 \$RPM_SOURCE_DIR/default.conf \$RPM_BUILD_ROOT/${php72_location}/etc/default.conf
install -D -m 0644 \$RPM_BUILD_DIR/${php72_filename}/php.ini-production \$RPM_BUILD_ROOT/${php72_location}/etc/php.ini

%post
id -u www >/dev/null 2>&1
[ $? -ne 0 ] && useradd -M -U www -d /home/www -s /sbin/nologin
chkconfig --add php72 >/dev/null 2>&1
[ $? -ne 0 ] && echo "[ERROR]: chkconfig --add php72"
/etc/init.d/php72 start
exit 0

%preun
/etc/init.d/php72 stop >/dev/null 2>&1
chkconfig --del php72 >/dev/null 2>&1
exit 0
...
rpmbuild_php72(){
    ...
    _create_fpm_config
    _create_spec
    rpmbuild -bb ~/rpmbuild/SPECS/php72.spec
}
