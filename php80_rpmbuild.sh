...
EOF
    sed -i "s|^prefix={php-fpm_location}$|prefix=${php80_location}|g" php80
}
...
_create_spec(){
    cat > ~/rpmbuild/SPECS/php80.spec << EOF
Name:           hws-php80
Version:        8.0.0
Release:        1%{?dist}
Summary:        PHP 8.0.0
...
%prep
tar zxf \$RPM_SOURCE_DIR/${php80_filename}.tar.gz

%build
cd ${php80_filename}
./configure --prefix=${php80_location} \
...
install -D -m 0755 \$RPM_SOURCE_DIR/php80 \$RPM_BUILD_ROOT/etc/init.d/php80
install -D -m 0644 \$RPM_SOURCE_DIR/default.conf \$RPM_BUILD_ROOT/${php80_location}/etc/default.conf
install -D -m 0644 \$RPM_BUILD_DIR/${php80_filename}/php.ini-production \$RPM_BUILD_ROOT/${php80_location}/etc/php.ini
...
rpmbuild_php80(){
    ...
    _create_fpm_config
    _create_spec
    rpmbuild -bb ~/rpmbuild/SPECS/php80.spec
}
