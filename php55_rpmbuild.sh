...
EOF
    sed -i "s|^prefix={php-fpm_location}$|prefix=${php55_location}|g" php55
}
...
_create_spec(){
    cat > ~/rpmbuild/SPECS/php55.spec << EOF
Name:           hws-php55
Version:        5.5.38
Release:        1%{?dist}
Summary:        PHP 5.5.38
...
%prep
tar zxf \$RPM_SOURCE_DIR/${php55_filename}.tar.gz

%build
cd ${php55_filename}
./configure --prefix=${php55_location} \
...
install -D -m 0755 \$RPM_SOURCE_DIR/php55 \$RPM_BUILD_ROOT/etc/init.d/php55
install -D -m 0644 \$RPM_SOURCE_DIR/default.conf \$RPM_BUILD_ROOT/${php55_location}/etc/default.conf
install -D -m 0644 \$RPM_BUILD_DIR/${php55_filename}/php.ini-production \$RPM_BUILD_ROOT/${php55_location}/etc/php.ini

%post
id -u www >/dev/null 2>&1
[ $? -ne 0 ] && useradd -M -U www -d /home/www -s /sbin/nologin
chkconfig --add php55 >/dev/null 2>&1
[ $? -ne 0 ] && echo "[ERROR]: chkconfig --add php55"
/etc/init.d/php55 start
exit 0

%preun
/etc/init.d/php55 stop >/dev/null 2>&1
chkconfig --del php55 >/dev/null 2>&1
exit 0
...
rpmbuild_php55(){
    ...
    _create_fpm_config
    _create_spec
    rpmbuild -bb ~/rpmbuild/SPECS/php55.spec
}
