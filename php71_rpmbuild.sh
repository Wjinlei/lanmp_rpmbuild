...
EOF
    sed -i "s|^prefix={php-fpm_location}$|prefix=${php71_location}|g" php71
}
...
_create_spec(){
    cat > ~/rpmbuild/SPECS/php71.spec << EOF
Name:           hws-php71
Version:        7.1.33
Release:        1%{?dist}
Summary:        PHP 7.1.33
...
%prep
tar zxf \$RPM_SOURCE_DIR/${php71_filename}.tar.gz

%build
cd ${php71_filename}
./configure --prefix=${php71_location} \
...
install -D -m 0755 \$RPM_SOURCE_DIR/php71 \$RPM_BUILD_ROOT/etc/init.d/php71
install -D -m 0644 \$RPM_SOURCE_DIR/default.conf \$RPM_BUILD_ROOT/${php71_location}/etc/default.conf
...
[ $? -ne 0 ] && echo "[ERROR]: chkconfig --add php71"
/etc/init.d/php71 start
exit 0

%preun
/etc/init.d/php71 stop >/dev/null 2>&1
chkconfig --del php71 >/dev/null 2>&1
exit 0
...
rpmbuild_php71(){
    ...
    _create_fpm_config
    _create_spec
    rpmbuild -bb ~/rpmbuild/SPECS/php71.spec
}
