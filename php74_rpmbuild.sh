...
EOF
    sed -i "s|^prefix={php-fpm_location}$|prefix=${php74_location}|g" php74
}
...
_create_spec(){
    cat > ~/rpmbuild/SPECS/php74.spec << EOF
Name:           hws-php74
Version:        7.4.13
Release:        1%{?dist}
Summary:        PHP 7.4.13
...
%prep
tar zxf \$RPM_SOURCE_DIR/${php74_filename}.tar.gz

%build
cd ${php74_filename}
./configure --prefix=${php74_location} \
...
install -D -m 0755 \$RPM_SOURCE_DIR/php74 \$RPM_BUILD_ROOT/etc/init.d/php74
install -D -m 0644 \$RPM_SOURCE_DIR/default.conf \$RPM_BUILD_ROOT/${php74_location}/etc/default.conf
install -D -m 0644 \$RPM_BUILD_DIR/${php74_filename}/php.ini-production \$RPM_BUILD_ROOT/${php74_location}/etc/php.ini

%post
id -u www >/dev/null 2>&1
[ $? -ne 0 ] && useradd -M -U www -d /home/www -s /sbin/nologin
chkconfig --add php74 >/dev/null 2>&1
[ $? -ne 0 ] && echo "[ERROR]: chkconfig --add php74"
/etc/init.d/php74 start
exit 0

%preun
/etc/init.d/php74 stop >/dev/null 2>&1
chkconfig --del php74 >/dev/null 2>&1
exit 0
...
rpmbuild_php74(){
    ...
    _create_fpm_config
    _create_spec
    rpmbuild -bb ~/rpmbuild/SPECS/php74.spec
}
