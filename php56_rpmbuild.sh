...
EOF
    sed -i "s|^prefix={php-fpm_location}$|prefix=${php56_location}|g" php56
}
...
_create_spec(){
    cat > ~/rpmbuild/SPECS/php56.spec << EOF
Name:           php56
Version:        5.6.40
Release:        1%{?dist}
Summary:        PHP 5.6.40
...
%prep
tar zxf \$RPM_SOURCE_DIR/${php56_filename}.tar.gz

%build
cd ${php56_filename}
./configure --prefix=${php56_location} \
...
install -D -m 0755 \$RPM_SOURCE_DIR/php56 \$RPM_BUILD_ROOT/etc/init.d/php56
install -D -m 0644 \$RPM_SOURCE_DIR/default.conf \$RPM_BUILD_ROOT/${php56_location}/etc/default.conf
...
rpmbuild_php56(){
    ...
    _create_fpm_config
    _create_spec
    rpmbuild -bb ~/rpmbuild/SPECS/php56.spec
}
