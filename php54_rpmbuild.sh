...
EOF
    sed -i "s|^prefix={php-fpm_location}$|prefix=${php54_location}|g" php54
}
...
_create_spec(){
    cat > ~/rpmbuild/SPECS/php54.spec << EOF
Name:           php54
Version:        5.4.45
Release:        1%{?dist}
Summary:        PHP 5.4.45
...
%prep
tar zxf \$RPM_SOURCE_DIR/${php54_filename}.tar.gz

%build
cd ${php54_filename}
./configure --prefix=${php54_location} \
...
install -D -m 0755 \$RPM_SOURCE_DIR/php54 \$RPM_BUILD_ROOT/etc/init.d/php54
install -D -m 0644 \$RPM_SOURCE_DIR/default.conf \$RPM_BUILD_ROOT/${php54_location}/etc/default.conf
install -D -m 0644 \$RPM_BUILD_DIR/${php54_filename}/php.ini-production \$RPM_BUILD_ROOT/${php54_location}/etc/php.ini
...
rpmbuild_php54(){
    ...
    _create_fpm_config
    _create_spec
    rpmbuild -bb ~/rpmbuild/SPECS/php54.spec
}
